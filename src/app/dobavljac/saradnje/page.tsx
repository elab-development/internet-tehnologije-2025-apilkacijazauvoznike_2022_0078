    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="border rounded-2xl p-4">
                <div className="font-semibold mb-3">Zahtevi koji čekaju</div>
                {incomingPending.length === 0 ? (
                    <div>Nema zahteva.</div>
                ) : (
                    <div className="space-y-2">
                        {incomingPending.map((s) => (
                            <div key={s.idSaradnja} className="flex items-center justify-between border rounded p-3">
                                <div>
                                    <div>
                                        <div className="font-medium">{s.uvoznikIme ?? "-"}</div>
                                        <div className="text-sm text-gray-600">{s.uvoznikEmail ?? "-"}</div>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <Button onClick={() => accept(s.idSaradnja)}>Prihvati</Button>
                                    <Button variant="danger" onClick={() => decline(s.idSaradnja)}>Odbij</Button>
                                </div>
                            </div>
                        ))}
                    </div>
